...
For a Shell fallback, read the exact `Latch Cursor current session id` from the
current prompt context; the `beforeSubmitPrompt` hook re-injected it from this
chat's payload. Set `LATCH_SESSION_ID` to that value. Never omit it or reuse an
id from another chat.
...
`bin/run_latch_gate.sh` with
`LATCH_SESSION_ID=<CURRENT_CURSOR_SESSION_ID>`, `LATCH_GATE_BACKEND`, and
`LATCH_MODEL_BACKEND` set.
