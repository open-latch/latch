#!/usr/bin/env bash
#
# latch_doctor.sh — thin wrapper around src/latch/install/doctor.py, the cross-platform
...
# mismatch / Rosetta SIGILL). All logic lives in src/latch/install/doctor.py.
#
# Usage:
...
#
# Interpreter resolution — must match src/latch/install/install_engine.py:resolve_python so
...
exec "$PY" "${KB_HOME}/src/latch/install/doctor.py" "$@"
