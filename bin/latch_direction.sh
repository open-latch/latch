#!/usr/bin/env bash
#
# latch_direction.sh — show the minimal project-direction layer.
set -euo pipefail
...
if [ -n "${LATCH_PYTHON:-}" ]; then
  ...
  echo "latch_direction: no Python found (set LATCH_PYTHON (legacy: CLAUDE_KB_PYTHON) to your interpreter)." >&2
  exit 2
fi
