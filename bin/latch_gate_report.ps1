...
if ($env:LATCH_PYTHON) {
  ...
  Write-Error "latch_gate_report: no Python found (set LATCH_PYTHON (legacy: CLAUDE_KB_PYTHON) to your interpreter)."
  exit 2
}

& $Py (Join-Path $KBHome "src/latch/gate/gate_report.py") --project (Get-Location).Path @args
exit $LASTEXITCODE
