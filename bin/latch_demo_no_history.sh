#!/usr/bin/env bash
#
# latch_demo_no_history.sh — run the public-safe no-history rejected-path demo.
set -euo pipefail
...
if [ -n "${LATCH_PYTHON:-}" ]; then
  ...
  echo "latch_demo_no_history: no Python found (set LATCH_PYTHON (legacy: CLAUDE_KB_PYTHON) to your interpreter)." >&2
  exit 2
fi
