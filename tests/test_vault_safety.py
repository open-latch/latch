...
def test_partial_or_forged_test_environment_is_refused(monkeypatch, tmp_path):
    ...
    with pytest.raises(paths.UnsafeTestExecutionError, match="incomplete"):
    ...
    with pytest.raises(paths.UnsafeTestExecutionError, match="does not match"):
        paths.project_dir("scope")
...
def test_direct_test_runner_refuses_before_pinned_vault_cleanup(tmp_path):
    ...
    assert "direct test execution is not allowed" in (result.stdout + result.stderr)
