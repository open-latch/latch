...
def test_clean_release_checkout_updates_to_exact_tag(tmp_path, monkeypatch):
    ...
    subprocess.run(["git", "clone", "--branch", "v0.1.0", str(remote), str(clone)], check=True, stdout=subprocess.DEVNULL)

    monkeypatch.setattr(update_latch, "ROOT", clone)
    monkeypatch.setattr(update_latch, "official_remote", lambda _url: True)
    monkeypatch.setattr(update_latch, "published_release_tag", lambda tag=None: tag or "v0.1.1")
    ...
    assert info["latest_tag"] == "v0.1.1"
    result = update_latch.apply_update("v0.1.1", dry_run=False)
    assert result["to_version"] == "0.1.1"
    assert _git(clone, "describe", "--tags", "--exact-match", "HEAD") == "v0.1.1"
