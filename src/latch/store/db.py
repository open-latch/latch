...
from latch.store import schema_version
from latch.store import vault_identity
from latch.store.paths import SCHEMA_PATH, db_path, ensure_project_dir
...
def connect(cwd: str | None = None) -> sqlite3.Connection:
    ...
    try:
        ...
        if had_nodes and installed_schema < schema_version.KB_SCHEMA_VERSION:
            ...
            # is due.
            from latch.store import vault_backup
...
def bump_focus_nc(
    ...
    *,
    ...
    if row["status"] == "stale":
        ...
        try:
            from latch.store import workstreams
