...
Usage:
    python src/latch/store/snapshot.py <project_dir_name>      # one project
    python src/latch/store/snapshot.py --all                   # every projects/<name>/ with a kb.db

Output: projects/<name>/kb.dump.sql

Restore:
    sqlite3 projects/<name>/kb.db < projects/<name>/kb.dump.sql
    python src/latch/store/migrate_v2.py <project_path>     # repopulates vec_nodes from embeddings
