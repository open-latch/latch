...
def _repair_cursor_wiring_from_mcp_startup() -> None:
    ...
    if os.environ.get("LATCH_ADAPTER") != "cursor":
        return
    try:
        from latch.hosts import cursor_wiring
...
def _repair_codex_wiring_from_mcp_startup() -> None:
    ...
    try:
        from latch.hosts import codex_wiring
...
def _run_proxy_or_continue_legacy() -> int | None:
    ...
    from latch.mcp import mcp_proxy
...
if __name__ == "__main__":
    from latch.mcp.windows_stdio import ensure_windows_standard_streams  # noqa: E402
...
# The verification-profile engine (src/latch/store/profiles.py — per-user gate presets
...
def prune_hidden_surface_tools(server: FastMCP) -> list[str]:
    ...
    """
    from latch.mcp.mcp_proxy import is_hidden_from_listing
...
if __name__ == "__main__":
    ...
    from latch.mcp.mcp_proxy import trimmed_tool_surface
