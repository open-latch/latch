...
Usage:
    python src/latch/install/install_engine.py [--python PATH] [--dry-run] [--check]
or via the wrappers:
...
LATCH_HOOK_MARKER = "/src/latch/hooks/"
LEGACY_LATCH_HOOK_MARKER = "/src/hooks/"
LATCH_HOOK_MARKERS = (LATCH_HOOK_MARKER, LEGACY_LATCH_HOOK_MARKER)
...
def seed_command_args(
    *,
    ...
    return [
        python_path,
        str(KB_HOME / "src" / "latch" / "pipeline" / "seed.py"),
        "--project",
        str(project),
        "--source",
        source,
...
def _is_latch_hook_entry(entry: dict) -> bool:
    for h in entry.get("hooks", []):
        if any(marker in (h.get("command") or "") for marker in LATCH_HOOK_MARKERS):
            return True
    return False
