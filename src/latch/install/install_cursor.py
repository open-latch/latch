...
from latch.hosts import cursor_hooks
from latch.hosts import cursor_rules_sync
from latch.install import install_engine
from latch.install.versioning import LATCH_VERSION, WIRING_VERSION
...
KB_HOME = Path(
    ...
    or next(p for p in Path(__file__).resolve().parents if p.name == "src").parent
)
...
CURSOR_COMPACT_ASSETS = (
    Path("src") / "latch" / "hosts" / "cursor_backend.py",
    Path("src") / "latch" / "hosts" / "cursor_compact.py",
    Path("src") / "latch" / "hosts" / "cursor_transcript.py",
...
def render_cursor_command(
    ...
    *,
    ...
    body = body.replace(
        f'python "{home}/src/latch/pipeline/maintenance.py"',
        f'LATCH_PYTHON="<CURSOR_MCP_PYTHON>" '
        f'LATCH_MAINTENANCE_BACKEND={backend} LATCH_MODEL_BACKEND={backend} '
        f'"<CURSOR_MCP_PYTHON>" "{home}/src/latch/pipeline/maintenance.py"',
    )
    body = body.replace(
        f'python "{home}/src/latch/gate/budget.py"',
        f'LATCH_PYTHON="<CURSOR_MCP_PYTHON>" '
        f'"<CURSOR_MCP_PYTHON>" "{home}/src/latch/gate/budget.py"',
    )
    if name == "latch-decay.md":
        body = body.replace(
            f'"<CURSOR_MCP_PYTHON>" "{home}/src/latch/pipeline/maintenance.py" "$PWD"',
            f'"<CURSOR_MCP_PYTHON>" "{home}/src/latch/pipeline/maintenance.py" weekly "$PWD"',
        )
...
def _check(args: argparse.Namespace, python_path: str, server_py: str) -> int:
    ...
    if args.with_hooks:
        checks.append(cursor_hooks.hooks_status(
            ...
            python_path,
            str(KB_HOME / "src" / "latch" / "hooks" / "cursor_session_start.py"),
            str(KB_HOME / "src" / "latch" / "hooks" / "cursor_before_submit.py"),
            str(KB_HOME / "src" / "latch" / "hooks" / "cursor_pre_tool_use.py"),
            str(KB_HOME / "src" / "latch" / "hooks" / "cursor_post_tool_use.py"),
        ))
...
def main(argv: list[str] | None = None) -> int:
    ...
    server_py = str((KB_HOME / "src" / "latch" / "mcp" / "mcp_server.py")).replace("\\", "/")
