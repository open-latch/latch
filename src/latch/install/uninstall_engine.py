...
Usage:
    python src/latch/install/uninstall_engine.py [--dry-run] [--check] [--purge]
                                   [--claude-md PATH ...] [--yes]
    python src/latch/install/uninstall_engine.py --cursor-only --cursor-project PATH
                                   [--dry-run | --check] [--yes]
or via the wrappers:
...
from latch.install import install_engine as ie
...
def strip_claude_md(targets: list[str], dry_run: bool) -> list[str]:
    ...
    if not targets:
        return changes
    from latch.hosts import claude_md_sync as cms
    for t in targets:
        path = Path(t)
        if dry_run:
