...
from latch.hosts import managed_doc_sync as mds
from latch.install.versioning import ROOT, WIRING_VERSION
...
def _manual(project: Path) -> str:
    if os.name == "nt":
        command = f'& "{sys.executable}" "{ROOT / "src" / "latch" / "install" / "install_codex.py"}" --yes'
    else:
        command = (
            f"{shlex.quote(sys.executable)} "
            f"{shlex.quote(str(ROOT / 'src' / 'latch' / 'install' / 'install_codex.py'))} --yes"
        )
    return (
...
def _owned_server_details(text: str) -> tuple[str, str]:
    ...
    from latch.install import install_codex
...
def _validate_owned_hooks(text: str) -> bool:
    ...
    from latch.hosts import codex_hooks
    ...
    try:
        ...
        if any(
            ...
            for name in codex_hooks.OWNED_HOOK_BASENAMES
    ...
    *,
    ...
    try:
        ...
        with _repair_lock(config):
            ...
            canonical_server = str(ROOT / "src" / "latch" / "mcp" / "mcp_server.py")
            ...
            if hooks.is_file():
                ...
                if owned_hooks:
                    desired_hooks, hook_changes = codex_hooks.merge_hooks(
                        current_hooks,
                        console_python,
                        str(ROOT / "src" / "latch" / "hooks" / "codex_session_start.py"),
                    )
