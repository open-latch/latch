...
from latch.hosts import codex_transcript
...
def main(argv: list[str] | None = None) -> int:
    ...
    server_py = str((install_codex.KB_HOME / "src" / "latch" / "mcp" / "mcp_server.py")).replace("\\", "/")
    hook_py = str((install_codex.KB_HOME / "src" / "latch" / "hooks" / "codex_session_start.py")).replace("\\", "/")
    checks = run_all(
