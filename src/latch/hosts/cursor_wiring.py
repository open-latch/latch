...
from latch.install import install_cursor
from latch.install.versioning import WIRING_VERSION
...
def _manual(project: Path) -> str:
    return (
        f"from `{project}`, run `"
        f"{shlex.quote(sys.executable)} {shlex.quote(str(install_cursor.KB_HOME / 'src' / 'latch' / 'install' / 'install_cursor.py'))} --yes`"
    )
...
def repair_project(project: str | Path) -> RepairResult:
    ...
    try:
        ...
        python_path, _installed_server, backend = _server_details(mcp_path)
        server_py = str(
            install_cursor.KB_HOME / "src" / "latch" / "mcp" / "mcp_server.py"
        )
        ...
        if _owned_hooks(hooks_path):
            ...
            desired_hooks, hook_changes = cursor_hooks.merge_hooks(
                current_hooks,
                python_path,
                str(install_cursor.KB_HOME / "src" / "latch" / "hooks" / "cursor_session_start.py"),
                str(install_cursor.KB_HOME / "src" / "latch" / "hooks" / "cursor_before_submit.py"),
                str(install_cursor.KB_HOME / "src" / "latch" / "hooks" / "cursor_pre_tool_use.py"),
                str(install_cursor.KB_HOME / "src" / "latch" / "hooks" / "cursor_post_tool_use.py"),
                path=hooks_path,
            )
