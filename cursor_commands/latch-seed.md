...
bash <KB_HOME>/bin/latch_seed.sh --source cursor --cursor-session-id "<CURSOR_SESSION_ID>"
```

```powershell
$env:LATCH_SEED_BACKEND = "<CURSOR_MODEL_BACKEND>"
$env:LATCH_MODEL_BACKEND = "<CURSOR_MODEL_BACKEND>"
& "<KB_HOME>/bin/latch_seed.ps1" --source cursor --cursor-session-id "<CURSOR_SESSION_ID>"
```
...
exactly `/latch-seed apply`, then rerun the same command with `--apply --yes`.
