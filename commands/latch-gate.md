...
layer) for the request in `$ARGUMENTS`. The primary MCP tool is `latch_gate`;
`kb_gate` remains a legacy alias. This slash is the manual escape hatch when
you want an explicit second opinion without relying on auto-invocation.
...
Legacy aliases: `/kb-gate` and `kb_gate` may exist on older installs and remain
supported for compatibility. Prefer `/latch-gate` and `latch_gate` in fresh
...
```bash
bash <KB_HOME>/bin/run_latch_gate.sh "$ARGUMENTS"
```
